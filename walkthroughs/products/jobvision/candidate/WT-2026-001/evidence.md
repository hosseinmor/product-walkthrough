...
- Permitted actions: ورود عبارت تست غیرحساس، انتخاب و حذف فیلترها، مرتب‌سازی، ناوبری و refresh؛ در ادامه و با مجوز صریح کاربر، ذخیره یک جستجوی تستی، فعال/غیرفعال‌کردن اعلان آن، حذف همان جستجوی تستی و پاک‌کردن تاریخچه جستجو نیز انجام شد.
...
- empty state
- ذخیره و حذف جستجوی تستی، فعال و غیرفعال‌کردن اعلان، صفحه مدیریت جستجوهای ذخیره‌شده و وضعیت کانال ایمیل
...
### E-022 — حذف جستجوی ذخیره‌شده با یک confirmation دومرحله‌ای انجام می‌شود

- Type: observed
- Timestamp: 2026-08-08T07:04:00Z
- Scope: نخستین رکورد `/saved-searches` با عنوان «حسابرسی»؛ حساب واردشده Production
- Conditions: این رکورد در همین واکترو به‌عنوان داده تستی ساخته شده و اعلان آن خاموش بود.
- Confidence: high
- Claim: کلیک «حذف» ابتدا dialog با پرسش «آیا از حذف این جستجوی ذخیره شده اطمینان دارید؟» و گزینه‌های «خیر» و «بله، مطمئنم» نمایش داد. پس از تأیید، رکورد «حسابرسی» حذف و شمار جستجوهای ذخیره‌شده از 3 به 2 رسید.
- Remaining uncertainty: حذف سایر رکوردها و رفتار در خطای شبکه آزموده نشد.
...
- یک recording کوتاه برای تحویل واقعی اعلان‌ها و recording جدا برای accessibility صفحه‌کلید/screen reader.

## خلاصه handoff

تکمیل نشده است. package همچنان `draft` است و همه evidence itemها به تصمیم Owner نیاز دارند. Apply عمداً به `WT-2026-002` واگذار شده است. جستجوی ذخیره‌شده تستی «حسابرسی» پس از confirmation حذف شد و سه رکورد recent search قبلی طبق مجوز کاربر پاک شده‌اند.
